...
func main() {
	...
	if *foh || *fohelp {
		...
		[1] -arp   => this flag will bring you instantly to arp [ root = YES]		
		usage: None => Type BOOL
		
		[2] -port  => this flag will give you th option to scan a host 				
		usage: go run main.go -port 192.168.189.120									
																					
		[4] -portl => this flag will give you the option to scan a LIST of hosts    
		usage: go run main.go -portl list.txt   								    
																					
		[5] -wifi => this flag will gice you the option to scan the area for BSSID's
		usage: None => Type: BOOL
		`
		fmt.Println(RED, help)
		os.Exit(1)
	}
	if *fohelpa {
		help := `
	...
	if *foport {
		...
		port.GetOpenPorts(host, port.PortRange{Start: 1, End: 8090})
		os.Exit(1)
	}
	if *fowifi {
		for {
